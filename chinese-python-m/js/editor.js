(function () {
  ...
  function fuzzyTextScore(query, text) {
    ...
    var pos = t.indexOf(q);
    if (pos === 0) return 10 + (t.length - q.length) * 0.01;
    if (pos >= 0) return 30 + pos + (t.length - q.length) * 0.01;
    ...
    return 60 + first + gaps * 2 + (t.length - q.length) * 0.01;
  }

  function autocompleteScore(item, query) {
    var fields = [
      [item.name, 0],
      [item.target, 12],
      [item.snippet, 18],
    ...
    for (var i = 0; i < fields.length; i++) {
      var score = fuzzyTextScore(query, fields[i][0]);
      if (score < 0) continue;
      score += fields[i][1];
  ...
  function updateAutocomplete() {
    ...
    function addCandidate(item) {
      ...
      seen[item.name] = 1;
      var score = autocompleteScore(item, word);
      if (score >= 0) ranked.push({ item: item, score: score });
    }
    FN_NAMES.forEach(function (n) { addCandidate(NAME_INDEX[n]); });
    KEYWORDS.forEach(function (k) {
      var b = NAME_INDEX[k];
      addCandidate(b ? b : { name: k, snippet: k, desc: '中文关键字' });
    });
    if (lastAnalysis) {
      lastAnalysis.variables.forEach(function (v) {
        addCandidate({ name: v.name, snippet: v.name, desc: '变量 · ' + v.type });
      });
    }
    if (!ranked.length) { hideAc(); return; }
    ranked.sort(function (a, b) {
      if (a.score !== b.score) return a.score - b.score;
      return a.item.name.length - b.item.name.length;
    });
    acItems = ranked.slice(0, 10).map(function (r) { return r.item; });
    acIndex = 0;
